...
void stopSteering() {
    sendCommandToPWM("0=0\n");
}

void stopThrottling() {
    sendCommandToPWM("1=0\n");
}
...
int main() {
    ...
    servoBlasterFile = fopen("/dev/servoblaster", "w");
    //g_usleep(50000);
    sleep(5);
    ...
    while((sock =socket(AF_INET,SOCK_STREAM,0))== -1){
        error("not opening socket");
    }

    server_addr.sin_family=AF_INET;
    server_addr.sin_port= htons(numPort);
    server_addr.sin_addr.s_addr = inet_addr(ipArray);
    ...
    while(g_atomic_int_get(&running)) {
        ...
        char command = recv_data[0];
        switch (command) {

            case '0':

                //direction
                directionstr[0] = recv_data[1];
                directionstr[1] = recv_data[2];
                direction = atoi(directionstr);
                printf("direction %d \n", direction);

                //speed
                speedstr[0] = recv_data[3];
                speedstr[1] = recv_data[4];
                speed = atoi(speedstr);
                printf("speed %d \n", speed);

                //TODO: Set target values
                targetRawSteeringValue = direction; //These need to be normalized with max value
                targetRawThrottleValue = speed; //These need to be normalized with max value
